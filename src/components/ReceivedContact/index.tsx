...
export default function ChatContent() {
    ...
    return (
        <>
            <Button size="small" onClick={() => setVisible(true)}>新联系人</Button>
            <Modal title="新请求"
                visible={visible}
                closable
                onCancel={() => setVisible(false)}
                footer={null}>
                <List itemLayout="horizontal" dataSource={received} renderItem={(item: any) => (
                    <Row>
                        <Col span={10}> {item.name}{item.remark?"------":""}{item.remark}</Col>
                        <Col span={4} offset={10}> <Button size="small" type="primary" onClick={handleClick(item.id)}>同意</Button></Col>
                    </Row>
                )} />
            </Modal>
        </>
